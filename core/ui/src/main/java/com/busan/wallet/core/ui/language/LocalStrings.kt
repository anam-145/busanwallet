...
data class Strings(
    ...
    val helpMiniAppTitle: String = "모듈 설치 및 관리",
    val helpMiniAppContent: String = "하단 네비게이션의 '허브' 탭에서 다양한 블록체인 지갑 모듈과 앱 모듈을 설치할 수 있습니다. 설치하고 싶은 모듈을 선택하여 '설치' 버튼을 누르면 메인 화면에 추가됩니다. 설치된 모듈은 메인 화면에서 바로 실행할 수 있습니다.",
    ...
    val faqQuestion1: String = "모듈이란 무엇인가요?",
    val faqAnswer1: String = "모듈은 Busan Wallet 내에서 독립적으로 실행되는 작은 애플리케이션입니다. 블록체인 모듈(Bitcoin, Ethereum 등)과 앱 모듈(부산일보, 비온미디어 등) 두 가지 종류가 있습니다. 각 모듈은 별도의 프로세스에서 실행되어 보안성과 안정성이 보장됩니다.",
    val faqQuestion2: String = "새로운 블록체인 지갑을 추가하려면 어떻게 하나요?",
    val faqAnswer2: String = "하단 네비게이션의 '허브' 탭에서 원하는 블록체인 모듈을 찾아 '설치' 버튼을 누르면 됩니다. 설치된 모듈은 메인 화면에 나타나며, 탭하여 바로 사용할 수 있습니다. 필요 없는 모듈은 언제든지 제거할 수 있습니다.",
    ...
    val faqAnswer4: String = "네, 가능합니다. Busan Wallet은 모듈형 구조로 설계되어 여러 블록체인 모듈을 동시에 설치하고 관리할 수 있습니다. 상단 헤더에서 현재 활성화된 블록체인을 확인하고, 탭하여 빠르게 전환할 수 있습니다.",
    val faqQuestion5: String = "모듈의 업데이트는 어떻게 하나요?",
    val faqAnswer5: String = "허브에서 설치된 모듈의 업데이트를 확인할 수 있습니다. 새 버전이 있으면 업데이트 버튼이 표시되며, 탭하여 최신 버전으로 업데이트할 수 있습니다.",
    val faqQuestion6: String = "오프라인에서도 사용할 수 있나요?",
    val faqAnswer6: String = "설치된 모듈은 오프라인에서도 기본 기능을 사용할 수 있습니다. 단, 잔액 조회나 트랜잭션 전송 같은 네트워크가 필요한 기능은 인터넷 연결이 필요합니다.",
...
val EnglishStrings = Strings(
    ...
    helpMiniAppTitle = "Installing and Managing Modules",
    helpMiniAppContent = "You can install various blockchain wallet modules and app modules from the 'Hub' tab at the bottom navigation. Select the module you want to install and press the 'Install' button to add it to the main screen. Installed modules can be launched directly from the main screen.",
    ...
    // FAQ Screen
    faqTitle = "FAQ",
    faqQuestion1 = "What is a module?",
    faqAnswer1 = "A module is a small application that runs independently within Busan Wallet. There are two types: blockchain modules (Bitcoin, Ethereum, etc.) and app modules (Busan Ilbo, Bion Media, etc.). Each module runs in a separate process, ensuring security and stability.",
    faqQuestion2 = "How do I add a new blockchain wallet?",
    faqAnswer2 = "Find the desired blockchain module in the 'Hub' tab at the bottom navigation and press the 'Install' button. Installed modules appear on the main screen and can be used immediately by tapping. Unnecessary modules can be removed at any time.",
    ...
    faqAnswer4 = "Yes, you can. Busan Wallet is designed with a modular structure, allowing you to install and manage multiple blockchain modules simultaneously. You can check the currently active blockchain in the header and tap to quickly switch.",
    faqQuestion5 = "How do I update modules?",
    faqAnswer5 = "You can check for updates of installed modules in the Hub. If a new version is available, an update button will appear, which you can tap to update to the latest version.",
    faqQuestion6 = "Can I use it offline?",
    faqAnswer6 = "Installed modules can use basic functions offline. However, functions that require network such as balance inquiry or transaction sending need internet connection.",
