...
import com.busan.wallet.feature.miniapp.common.domain.usecase.ConnectToServiceUseCase
import com.busan.wallet.feature.miniapp.common.domain.usecase.LoadMiniAppManifestUseCase
import com.busan.wallet.feature.miniapp.common.domain.usecase.RequestTransactionUseCase
import com.busan.wallet.feature.miniapp.webapp.domain.usecase.GetActiveBlockchainIdFromWebAppServiceUseCase
// Credential 기능 비활성화
// import com.busan.wallet.feature.miniapp.webapp.domain.usecase.GetCredentialsUseCase
// import com.busan.wallet.feature.miniapp.webapp.domain.usecase.CreateVPFromServiceUseCase
...
import com.busan.wallet.core.common.extension.resolveEntryPoint
import com.google.gson.Gson
import com.google.gson.reflect.TypeToken
// import com.busan.wallet.feature.miniapp.webapp.domain.model.CredentialInfo
...
class WebAppViewModel @Inject constructor(
    ...
    private fun connectToService() {
        viewModelScope.launch {
            when (val result = connectToServiceUseCase.connect(_uiState.value.appId)) {
                is MiniAppResult.Success -> {
    ...
    private fun requestTransaction(transactionData: JSONObject) {
        ...
        viewModelScope.launch {
            ...
            try {
                // 활성 블록체인 정보 가져오기
                val activeBlockchainId = getActiveBlockchainIdUseCase()
                if (activeBlockchainId == null) {
            ...
            try {
                ...
                when (val result = requestTransactionUseCase(request)) {
                    is MiniAppResult.Success -> {
                        ...
                        _effect.emit(
                            WebAppContract.Effect.SendTransactionResponse(responseJson)
                        )
                    }
                    is MiniAppResult.Error -> {
    ...
    private fun checkActiveBlockchain() {
        viewModelScope.launch {
            val blockchainId = getActiveBlockchainIdUseCase()
