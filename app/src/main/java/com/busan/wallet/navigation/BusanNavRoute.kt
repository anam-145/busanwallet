 ...
 * ## 사용 예시:
 * // ✅ 타입 안전한 네비게이션
 * navController.navigate(BusanNavRoute.Main.route)  // 자동완성 지원
 * 
 * // ✅ 안전한 파라미터 처리
 * navController.navigate(
 *     BusanNavRoute.MiniAppDetail.createRoute(appId)  // "miniapp/123"
 * )
 * 
 * // ❌ 기존 방식의 문제점
...
sealed class BusanNavRoute(
     ...
     * 아래는 sealed class BusanNavRoute(val route: String)를 상속 받는 하위 클래스들
     * */
    data object Main : BusanNavRoute("main")
    
    /** 허브 화면 - 미니앱 및 서비스 목록 */
    data object Hub : BusanNavRoute("hub")
    
    /** 브라우저 화면 - 웹 브라우징 */
    data object Browser : BusanNavRoute("browser")
    
    // DID 기능 임시 비활성화
    // /** 신원(Identity) 화면 - DID/VC 관리 */
    // data object Identity : BusanNavRoute("identity")
    
    /** 설정 화면 - 앱 설정 및 프로필 */
    data object Settings : BusanNavRoute("settings")
    ...
    * navController.navigate(
    *     BusanNavRoute.MiniAppDetail.createRoute("weather-app")
    * )
    * // 결과: "miniapp/weather-app"
    */
    data object MiniAppDetail : BusanNavRoute("miniapp/{appId}") {
        /** 특정 미니앱으로 이동하기 위한 경로 생성 */
        fun createRoute(appId: String) = "miniapp/$appId"
    }
    
    /** 모듈 상세 화면 (블록체인 모듈 등) */
    data object ModuleDetail : BusanNavRoute("module/{moduleId}") {
        /** 특정 모듈로 이동하기 위한 경로 생성 */
        fun createRoute(moduleId: String) = "module/$moduleId"
    }
    
    /** 학생증 상세 화면 */
    data object StudentCardDetail : BusanNavRoute("student_card")
    
    /** 운전면허증 상세 화면 */
    data object DriverLicenseDetail : BusanNavRoute("driver_license")
    
    // ========== 인증 화면들 ==========
    // 로그인 및 비밀번호 설정 화면들
    
    /** 로그인 화면 - 비밀번호 입력 */
    data object Login : BusanNavRoute("auth/login")
    
    /** 비밀번호 설정 화면 - 최초 설정 */
    data object SetupPassword : BusanNavRoute("auth/setup")
    ...
    data object Help : BusanNavRoute("settings/help")
    
    @Deprecated("Use SettingsRoute.FAQ instead", level = DeprecationLevel.WARNING)
    data object FAQ : BusanNavRoute("settings/faq")
    
    @Deprecated("Use SettingsRoute.AppInfo instead", level = DeprecationLevel.WARNING)
    data object AppInfo : BusanNavRoute("settings/appinfo")
    
    @Deprecated("Use SettingsRoute.License instead", level = DeprecationLevel.WARNING)
    data object License : BusanNavRoute("settings/license")

    
    companion object {
        /**
         * 주어진 route 문자열로 해당하는 BusanNavRoute 객체를 찾습니다.
         * 
         * @param route 네비게이션 route 문자열
         * @return 해당하는 BusanNavRoute 객체, 없으면 null
         */
        fun fromRoute(route: String?): BusanNavRoute? {
            return when (route) {
                Main.route -> Main
                Hub.route -> Hub
